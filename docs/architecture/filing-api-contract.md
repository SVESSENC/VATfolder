...
- Body (JSON):
  - `taxpayerId` (string)
  - `period` (string, for example `"2026-Q1"`)
...
- `eventId`: UUID
- `eventType`: for example `FilingSubmitted`, `FilingValidated`, `AssessmentCompleted`, `AmendmentSubmitted`, `ClaimSubmitted`, `AuditRecorded`
- `timestamp`
- `correlationId`
