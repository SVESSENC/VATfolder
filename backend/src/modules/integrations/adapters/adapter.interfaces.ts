...
export interface IMitIdAdapter extends IAdapter {
  ...
  exchangeCodeForToken(code: string): Promise<{ token: string; userInfo: MitIdUserInfo }>;
...
export interface MitIdUserInfo {
  ...
  email?: string;
  name?: string;
  cvr?: string; // For organisation flows
}
...
export interface ISkatAdapter extends IAdapter {
  ...
  validatePayload(payload: SkatSubmissionPayload): Promise<{ valid: boolean; errors?: string[] }>;
}
